...
依据是 `docs/DESIGN.md`、`docs/SCHEMA.md`、`docs/API.md`。云上部署和恢复演练记录在 `docs/ACCEPTANCE.md`。录入真实学生前换成可信证书。
...
- [x] 积分模板：全局模板、班内覆盖、班内私有模板
- [x] 卫生服务：开始轮次、计次、未推椅子、`1/3 → 1/4`、冻结候选、抽选、取消不重抽、确认、纠正失效、关轮
- [x] 排行榜 `GET /leaderboard`，并列名次为 `1, 1, 3`
- [x] 回放：时间轴、帧、任意时刻状态；检查点每 200 个回放事件加每日兜底，不进入记分事务
- [x] 点名与倒计时，并用 SSE 同步到其他端
- [x] Excel 导出：花名册可再导入，积分明细和榜单与列表筛选一致
- [x] 前端页面：座位图、手机三步换座、记分、卫生、榜单回放、全屏展示、离线禁写

## 5. 运维脚本

- [x] 迁移命令，以及创建教师、重置密码、重算余额
- [x] 回放检查点任务
- [x] 每日 `pg_dump` 记录与失败告警
- [x] 匿名化账本导出失败后的重试，以及恢复旧备份后的补做命令
...
- [x] 在云上用自签证书和一份 dump 做恢复演练。真实学生数据仍等学校的可信证书

## 8. 页面尚未接通

接口已经注册。老师在页面上还不能做这些事，步骤见 `docs/使用教程.md`，路由见 `docs/API.md`「页面是否调用」。

- [x] 建学期、激活学期、建班、改名和归档
- [x] 名单导入，以及单个学生的新增、修改、离班、恢复
- [x] 机房布局调整
- [x] 在页面上修改密码
- [x] 原因模板和普通标记的维护
- [x] 审计、备份记录和备份下载
- [x] 花名册和积分明细导出。榜单页已经有导出按钮
- [x] 学生匿名化和班级匿名化
